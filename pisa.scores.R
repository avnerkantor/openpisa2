updateSelectInput(session, "Country1", choices = unique(countries$Country), selected = "Israel")
updateSelectInput(session, "Country2", choices = unique(countries$Country), selected = "Singapore")
  ...
  scoresPlotFunction<-function(country){
    ...
    gg<-ggplot(plotData3, aes(x=Year, y=Average, colour=GenderESCS, text=round(Average))) +
      ...
      scale_y_continuous(
        ...
        breaks=unlist(SubjectExpertiseLevels[2]),
        labels=unlist(SubjectExpertiseLevels[1]),
        #limits = c(200,700),
